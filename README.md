...
* **Библиотеки:** *typing*, *collections*, *random*, *time*, *functools*
* **Допущения:**
  ...
  - Время выводится в микросекундах с обрезанием до 4-го знака
...
## Интересные тест-кейсы
 ...
 - При использовании например ключа `lambda x: x % 3` на массиве `[7, 5, 8, 2, 4]` в функции `heap_sort` числа отсортируются ввиде `[4, 7, 8, 2, 5]`, а функция `sort()` выдаст `[7, 4, 5, 8, 2]`. То есть сортировка правильная, но для одинаковых по ключу элементов сортировка чуть разная
...
### ИСПАНЦЫ!!!
Можно отключить красивый вывод при помощи замены переменной `pretty` на значение `False` (32 строка в main)
