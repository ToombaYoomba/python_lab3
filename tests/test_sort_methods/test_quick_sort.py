...
def test_quick_sort(inp, expectation):
    with expectation:
        assert quick_sort(inp) == sorted(inp)
...
def test_quick_sort_key_cmp(inp, key_custom, cmp_custom, expectation):
    with expectation:
        result = quick_sort(inp, key_custom=key_custom, cmp_custom=cmp_custom)
